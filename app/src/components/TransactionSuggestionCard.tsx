...
export function TransactionSuggestionCard({ payout, suggestion }: Props) {
  ...
  const totalCredit = rows.reduce((prev, curr) => prev + (curr.credit ?? 0), 0);
  const totalDebit = rows.reduce((prev, curr) => prev + (curr.debit ?? 0), 0);
        ...
        <table className="mt-4 w-full border-collapse border border-gray-300 text-sm">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td colSpan={2} className="border border-gray-300 pl-4">
                {diff == 0
                  ? 'Credit and debit are in blance'
                  : `Not balancing (${diff} ${payout.currency_code})`}
              </td>
